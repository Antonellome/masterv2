...
export const createNewUser = functions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
...
export const onTecnicoStatusChange = functions.firestore
    ...
    .onUpdate(async (change: Change<DocumentSnapshot>) => {
        ...
        if (newData.stato === previousData.stato || !uid) return null;
...
export const sendAndFanOutNotification = functions.firestore
    ...
    .onCreate(async (snap: DocumentSnapshot) => {
        ...
        const targetUids: string[] = [...to_uids];
        ...
        if (to_categories.length > 0) {
            ...
            snapshot.forEach((doc: admin.firestore.QueryDocumentSnapshot) => {
                ...
                if (tecnico.uid) targetUids.push(tecnico.uid);
                if (tecnico.fcmToken) fcmTokens.push(tecnico.fcmToken);
            });
        }

        const uidsForTokenFetch = to_uids.filter((uid: string) => !targetUids.includes(uid));
        if (uidsForTokenFetch.length > 0) {
            const promises = uidsForTokenFetch.map((uid: string) => db.collection('tecnici').where('uid', '==', uid).limit(1).get());
        ...
        const pushPromise = (() => {
            if (uniqueTokens.length === 0) return Promise.resolve();
...
const manageScadenza = async (
    idRiferimento: string, 
    tipoScadenza: string, 
    tipoOggetto: 'Veicoli' | 'Documenti',
    descrizione: string,
    data: admin.firestore.Timestamp | undefined
) => {
...
export const onVeicoloWritten = functions.firestore
    ...
    .onWrite(async (change: Change<DocumentSnapshot>) => {
        ...
        if (!change.after.exists) return; // Gestito da onDelete
...
export const onVeicoloDelete = functions.firestore
    .document('veicoli/{veicoloId}')
...
export const onDocumentoWritten = functions.firestore
...
export const onDocumentoDelete = functions.firestore
    .document('documenti/{documentoId}')
    .onDelete(async (snap: DocumentSnapshot) => {
        const docId = snap.id;
