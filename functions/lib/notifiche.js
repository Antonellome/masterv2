...
exports.deleteNotifiche = exports.sendNotifica = exports.markNotificheAsRead = exports.getNotifiche = void 0;
...
exports.getNotifiche = functions.onCall({ region: "europe-west6" }, async (request) => {
...
exports.markNotificheAsRead = functions.onCall({ region: "europe-west6" }, async (request) => {
    const db = admin.firestore(); // Inizializzazione LATE (pigra)
    if (!request.auth) {
        throw new functions.HttpsError("unauthenticated", "Autenticazione richiesta.");
    }
...
exports.sendNotifica = functions.onCall({ region: "europe-west6" }, async (request) => {
    const db = admin.firestore(); // Inizializzazione LATE (pigra)
...
exports.deleteNotifiche = functions.onCall({ region: "europe-west6" }, async (request) => {
    const db = admin.firestore(); // Inizializzazione LATE (pigra)
    // Aggiungere un controllo per soli admin
    const ids = request.data.notificationIds;
    if (!Array.isArray(ids) || ids.length === 0) {
