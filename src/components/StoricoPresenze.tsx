...
const StoricoPresenze: React.FC<StoricoPresenzeProps> = ({ selectedDate }) => {
    ...
    // Filtra i rapportini in base alla data selezionata DIRETTAMENTE dallo store
    const rapportiniDelGiorno = useMemo(() => {
        if (!selectedDate) return [];
        return tuttiIRapportini.filter(r => {
            const dataRapportino = r.dataInizio || r.data;
            if (!dataRapportino) return false;
            const d = typeof dataRapportino.seconds === 'number' ? dayjs(new Date(dataRapportino.seconds * 1000)) : dayjs(dataRapportino);
            return d.isSame(selectedDate, 'day');
        });
    }, [selectedDate, tuttiIRapportini]);
    ...
    const aggregateData = useMemo(() => {
        ...
        const oreTotali = rapportiniDelGiorno.reduce((acc, curr) => acc + (curr.oreLavoro || 0), 0);
        ...
        for (const rapportino of rapportiniDelGiorno) {
            ...
            for (const tecnicoId of allTecniciInRapportino) {
                ...
                if (tecnico && tipoGiornata) {
                    ...
                    if (TIPI_GIORNATA_GIUSTIFICATIVI.some(keyword => tipoGiornataLower.includes(keyword))) {
    ...
    const handleCreaRapportino = (tecnico: Tecnico) => {
        if (!selectedDate) return;
        navigate(`/rapportino/edit/new?tecnicoId=${tecnico.id}&data=${selectedDate.toISOString()}`);
    };
    ...
    return (
        <Box>
            <Grid container spacing={2} sx={{ mb: 4 }}>
                ...
                <AnalysisKPIBox title="Costo Totale Stimato" value={aggregateData.costoTotale} icon={<EuroSymbolIcon color="primary" />} />
            </Grid>
                    ...
                    <Paper variant="outlined" sx={{ p: 1, maxHeight: 300, overflowY: 'auto' }}>
                        <List dense>
                            {mancantiAttivi.map(tecnico => (
                                <ListItem key={tecnico.id} secondaryAction={<Button size="small" variant="outlined" onClick={() => handleCreaRapportino(tecnico)}>Crea R.</Button>}>
                        ...
                        <List dense>
                            {operativi.map(({ tecnico, tipoGiornata }) => (
                                <ListItem key={tecnico.id}>
                                     <ListItemAvatar><Avatar sx={{ width: 32, height: 32, bgcolor: 'success.light' }}><CheckCircleOutlineIcon fontSize="small" /></Avatar></ListItemAvatar>
                                    <ListItemText primary={`${tecnico.cognome} ${tecnico.nome}`} />
                     ...
                     <Paper variant="outlined" sx={{ p: 1, maxHeight: 300, overflowY: 'auto' }}>
                        <List dense>
                            {assentiGiustificati.map(({ tecnico, tipoGiornata }) => (
                                <ListItem key={tecnico.id}>
                                    <ListItemAvatar><Avatar sx={{ width: 32, height: 32, bgcolor: 'info.light' }}><HelpOutlineIcon fontSize="small" /></Avatar></ListItemAvatar>
                                    <ListItemText primary={`${tecnico.cognome} ${tecnico.nome}`} />
                        ...
                        <List dense>
                            {assentiNonAttivi.map(tecnico => (
                                <ListItem key={tecnico.id}>
                                    ...
                                    <ListItemText primary={`${tecnico.cognome} ${tecnico.nome}`} secondary="Non attivo"/>
                                </ListItem>
                            ))}
