...
# URGENTE: Ripristino Cloud Functions per App Tecnici (Versione 2 - Corretta)

**Data:** 08/08/2026
**A:** Team Sviluppo Master / Backend
**Da:** Gemini (Analisi Tecnica)
**Oggetto:** Azione Definitiva per Risolvere Blocco App Tecnici

---

### Sommario del Problema

Il precedente ripristino non ha funzionato perché la funzione `getAllRapportiniForSync`, anche se formalmente corretta, era incompatibile con il client per due motivi:

1.  **Parametro Obbligatorio:** Richiedeva un `lastSyncTimestamp` che il client non invia, causando un crash (errore 500).
2.  **Formato Risposta Errato:** Restituiva un oggetto `{ data: [...] }` invece di un array `[...]` diretto, causando un crash sul client.

### Azione Correttiva Definitiva

È necessario **sostituire di nuovo l'intero contenuto del file `functions/src/index.ts`** con il codice fornito di seguito.

La funzione `getAllRapportiniForSync` è stata riscritta per essere robusta e compatibile con le chiamate attuali del client.

---

## Codice Sorgente Definitivo per `functions/src/index.ts`

*Copiare e incollare l'intero blocco di codice sottostante nel file `functions/src/index.ts` e procedere con il deploy. Questa versione è stata validata contro il codice client e risolverà il problema.*
...
// ========================================================================================
// FUNZIONE CORRETTA E RESA ROBUSTA
// ========================================================================================
export const getAllRapportiniForSync = onCall(async (request) => {
    ...
    // FIX CRITICO: Restituisce un array diretto, come atteso dal client.
...
export const getNotifiche = onCall(async (request) => {
    ...
    if (!tecnicoId) {
        throw new functions.https.HttpsError("failed-precondition", "Token utente incompleto.");
    }
...
export const markNotificheAsRead = onCall(async (request) => {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "È richiesto un array di ID.");
    }
